					...
					const char *text, const char *extra)
{
	...
	#if(HCI_EVT_LOG_CT) // the original branch
	if (ts_len > 0) {
		printf("%s", line);
		if (len < col)
		...
		printf("%s\n", line);
	#else
		#warning "Some log ouput is commented here";
	#endif
}
...
static void le_adv_report_evt(uint16_t index, const void *data, uint8_t size)
{
	...
	#else
		//DBG_LOG_WARN("the report adv report evt num = %d", evt->num_reports);
	#endif
report:
	#if(0) // the original
	...
	#if(1)// to report the ADV report event
		union pipe_msg_info kp_pmsg = {0};
		if(ST_OK == app_hci_fill_adv_report_info( evt, &kp_pmsg))
		{
			app_hci_report_event( app_hci_get_pipefd_for_writting(), P_MSG_ADV_REPORT, (const union pipe_msg_info *)&kp_pmsg);
		}
	#endif
