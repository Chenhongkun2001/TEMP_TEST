...
static int open_socket(uint16_t channel)
{
	...
	if (fd < 0) {
		...
		DBG_LOG_ERR("fail to open channel");
		return -1;
	}
	...
	if (bind(fd, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
		...
		DBG_LOG_ERR("fail to bind the channel");
		close(fd);
		return -1;
	}
...
static int open_channel(uint16_t channel)
{
	...
	if (data->fd < 0) {
		free(data);
		DBG_LOG_ERR("fail to open socket channel %d", channel);
		return -1;
	}
	...
	if (mainloop_add_fd(data->fd, EPOLLIN, data_callback,
		...
		close(data->fd);
		free(data);
		DBG_LOG_ERR("mainloop_add_fd fail %d", ENOSYS);
		return -1;
	};
	#else
	int tpint = 0;
	tpint = mainloop_add_fd(data->fd, EPOLLIN, data_callback,data, free_data);
	if (tpint < 0) 
	{
		close(data->fd);
		free(data);
		DBG_LOG_ERR("mainloop_add_fd fail %d, tpint = %d", ENOSYS, tpint);
		return -1;
	};
	#endif
...
int control_tracing(void)
{
	...
	if (open_channel(HCI_CHANNEL_MONITOR) < 0) {
		if (!hcidump_fallback)
		{
			DBG_LOG_ERR("hcidump_fallback is %d", hcidump_fallback);
			return -1;
		}
		if (hcidump_tracing() < 0)
		{
			DBG_LOG_ERR("hcidump_tracking fail");
			return -1;
		}
		return 0;
	}
