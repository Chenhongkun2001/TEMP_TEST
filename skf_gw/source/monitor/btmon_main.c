...
#include "control.h"
#include "display.h"
#if (SKF_GW_INTEGRATION == 1)
#include "util_dbg.h"
#endif /* SKF_GW_INTEGRATION == 1 */
DBG_LOCAL_LOG_DEBUG
...
#else /* SKF_GW_INTEGRATION == 1 */
	DBG_LOG_INFO("btmon - Bluetooth monitor\n"
		"Usage:\n");
	DBG_LOG_INFO("\tbtmon [options]\n");
	DBG_LOG_INFO("options:\n"
		"\t-r, --read <file>      Read traces in btsnoop format\n"
		"\t-w, --write <file>     Save traces in btsnoop format\n"
		"\t-a, --analyze <file>   Analyze traces in btsnoop format\n"
		"\t-s, --server <socket>  Start monitor server socket\n"
...
int btmon_main(int argc, char *argv[])
{
...
#if (SKF_GW_INTEGRATION == 1)
				DBG_LOG_ERR("Socket name too long\n");
#else /* SKF_GW_INTEGRATION == 1 */
...
#if (SKF_GW_INTEGRATION == 1)
				DBG_LOG_ERR("Color option must be one of "
						"auto/always/never\n");
#else /* SKF_GW_INTEGRATION == 1 */
	...
	if (argc - optind > 0) {
#if (SKF_GW_INTEGRATION == 1)
		DBG_LOG_ERR("invalid cmd line parameter\n");
#else /* SKF_GW_INTEGRATION == 1 */
...
#if (SKF_GW_INTEGRATION == 1)
		DBG_LOG_ERR("Display and analyze can't be combined\n");
#else /* SKF_GW_INTEGRATION == 1 */
...
#if (SKF_GW_INTEGRATION == 1)
	DBG_LOG_INFO("Bluetooth monitor ver %s\n", "5.7.6");
#else /* SKF_GW_INTEGRATION == 1 */
	...
	{
#if (SKF_GW_INTEGRATION == 1)
		DBG_LOG_ERR("jlink fail, tty = %d, jlink = %d", tty, jlink);
#endif /* SKF_GW_INTEGRATION == 1 */
		return EXIT_FAILURE;
	}	
