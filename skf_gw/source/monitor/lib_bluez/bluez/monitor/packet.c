...
report:
	...
	#if(1)// modified by Victor for test
		print_field("_____the following is advertising packet_len=%d", evt->data_len);
		packet_hexdump( evt->data, evt->data_len);
		print_field("_________________________________________________");
	#endif
}
