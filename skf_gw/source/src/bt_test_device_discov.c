				...
				struct l_dbus_message *msg, void *user_data)
{
		...
		{ // to stop BT scanning when some device is connected
			...
			{ // to turn off BT scanning
				...
				// Comment 20240628: Scannning would be turned off in BT_EVENT_DISCONNECTION (app_pro_ble.c)
				//bt_common_turn_off_scanning(pt_app_mgt->hld_proxy_adapter1, NULL);
			}
			else
			{
				//bt_common_set_powered( pt_app_mgt->hld_proxy_adapter1, true);
				// Comment 20240628: Scannning would be turned on in BT_EVENT_CONNECTION (app_pro_ble.c)
...
int main(void)
{
	...
	l_dbus_client_set_ready_handler( ptclient, bluez_client_ready, NULL, NULL);
		...
		pt_timer = l_timeout_create(10, sys_timer_callback, NULL, sys_timer_destroy_callback);
	#endif	
