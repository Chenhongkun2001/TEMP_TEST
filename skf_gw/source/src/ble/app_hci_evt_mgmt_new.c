#include "app_hci_evt_mgmt_new.h"
#include "util_dbg.h"
...
/**
 * @brief initialize of pt_dtpacket_saving_ctr
 */
bool 
app_hci_init_dtpacket_saving_ctr(
...
#if (0)	// comment it out as not used
...
#endif
struct 
sit_dtpacket_saving_controller*app_hci_get_dtpacket_saving_ctr(void)
{
...
/**
 * @brief append data-packet to queue (for
 *        Insight-T)
 * @param pt_dtpacket_saving_ctr: The object to save the white list of
 *        Insight-T
 * @param pt_adv_rpt: The list of MAC address
 * @param pt_ad_pkt: The list of name ID
 * @return ST_OK if append success, ST_ERR if else
 */
static app_state_t
app_hci_append_dtpacket(
  struct sit_dtpacket_saving_controller*pt_dtpacket_saving_ctr,
  struct msg_adv_report*pt_adv_rpt,
  struct sit_ad_packet*pt_ad_pkt)
{
...
/**
 * @brief try to write Insight-T advertising data to database(for
 *        Insight-T)
 * @param pt_dtpacket_saving_ctr: The object to save the white list of
 *        Insight-T
 * @param pt_adv_rpt: The list of MAC address
...
/**
 * @brief create a pipe for P_MSG_ADV_REPORT type IPC
 * g_pipefd_hci_event_report[0]: read pipe-fd for app_pro_general_new process
 * g_pipefd_hci_event_report[1]: write pipe-fd for btmon_main process
 */
app_state_t
  app_hci_create_pipe_fds(void)
{
	app_state_t tpret = ST_OK;
	int tpint = 0;
...
/**
 * @brief write P_MSG_ADV_REPORT type data into pipe-fd for writing(for
 *        Insight-T)
 * @param kpfd: the pipe-fd for data writting
 * @param mtype: msg type. P_MSG_ADV_REPORT
 * @param pt_pmsg: The pointer to pipe_msg
 * @return : ST_OK(0): success ST_ERR(1): fail
 */
app_state_t
app_hci_report_event(
  int kpfd,
  enum pipe_msg_type mtype,
  const union pipe_msg_info *pt_pmsg)
{
	app_state_t tpret = ST_OK;
...
/**
 * @brief copy P_MSG_ADV_REPORT data from pt_evtinfo into pt_pmsg for writing(for
 *        Insight-T)
 * @param pt_evtinfo: source of P_MSG_ADV_REPORT data
 * @param pt_pmsg: destination of P_MSG_ADV_REPORT data
 * @return : ST_OK(0): success ST_ERR(1): fail
 */
app_state_t
app_hci_fill_adv_report_info(
  const struct bt_hci_evt_le_adv_report*pt_evtinfo,
  union pipe_msg_info *pt_pmsg)
{
	app_state_t tpret = ST_OK;
	int32_t tpi32 = 0;
...
/**
 * @brief check crc validation(for
 *        Insight-T)
 * @param pt_pmsg: P_MSG_ADV_REPORT type msg
...
/**
 * @brief filter P_MSG_ADV_REPORT type msg by white list (for
 *        Insight-T)
 * @param pt_pmsg: P_MSG_ADV_REPORT type msg
 * @param returnedNameId: NameId of filtered msg
 * @return : true: filter msg successfully with returnedNameId being set accordingly
 *           false: filter msg fail
 */
...
/**
 * @brief decode manufacture data
 */
static bool
to_decode_eir_manufacture_data(
  struct sit_ad_packet *pt_ad_pkt,
...
/**
 * @brief handler to deal with HCI event report.
 */
void *
thandler_hci_event(void *para)
{
	...
	{	
		...
		continue;
			...
			app_hci_save_sit_adv_data_all( pt_dtpacket_saving_ctr);
		#endif
		...
		{
			// DBG_LOG_WARN("TODO=====the msg data not match\n");
			continue;
		}
