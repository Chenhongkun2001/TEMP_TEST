/**
 * @file    sensor_interface_insight_predict_pro.c
 * @author  Xiaoyuan (Sean) Ma
 * @date    2025-03-24
 * @brief   Implementation of Insight Predict Pro Interfaces.
 * @details
 */
#include <pthread.h>
#include <fcntl.h>
#include "sensorInterface/sensor_interface_insight_predict_pro.h"
...
#define INSIGHT_PREDICT_PRO_WAVEFORM_DATA_START (0)
#define INSIGHT_PREDICT_PRO_WAVEFORM_DATA_END (5)
#define INSIGHT_PREDICT_PRO_DATA_COLLECT_MAX_NUMBER (3)
#define INSIGHT_PREDICT_PRO_CONFIG_RETRIEVE_MAX_NUMBER (3)
#define INSIGHT_PREDICT_PRO_CONFIG_DISSEM_MAX_NUMBER (3)
...
#if (1)
  SKFChina_Common_SensorType_ADXL382_VIBRATION,
  SKFChina_Common_SensorType_ADXL382_VIBRATION,
  SKFChina_Common_SensorType_ADXL382_VIBRATION,
  SKFChina_Common_SensorType_ADXL382_VIBRATION,
  SKFChina_Common_SensorType_ADXL382_VIBRATION,
...
protobuf_codec_data_pair_ele_data_t *bulkDataPro;
...
/**
 * @brief Register a sensor interface (instance) for InsightPredictPro. An
 ...
 */
bool
registerSensorIntfInsightPredictPro(
  struct sensor_interface *sensorIntf,
  uint8_t *errCode)
{
...
/**
 * @brief Unregister a sensor interface (instance) for InsightPredictPro. The
 ...
 */
bool
unregisterSensorIntfInsightPredictPro(
  struct sensor_interface *sensorIntf,
  uint8_t *errCode)
{
 ...
 * InsightPredictPro. In this routine, 1) current time would be set to the
...
/**
 * @brief This is the data collection instance of InsightPredictPro. In this
  ...
  uint8_t *err_code)
{
  ...
  _attempt = 0;
  for(uint32_t i = INSIGHT_PREDICT_PRO_WAVEFORM_DATA_END + 1,
      delta = INSIGHT_PREDICT_PRO_DATA_COLLECT_MAX_NUMBER;
      i < sizeof(collectDataGroup) / sizeof(collectDataGroup[0]);
      )
  {
    if((i + INSIGHT_PREDICT_PRO_DATA_COLLECT_MAX_NUMBER) >
...
_SHORT_DATA_RETRY:
  ...
  _attempt = 0;
  for(uint32_t i = INSIGHT_PREDICT_PRO_WAVEFORM_DATA_START;
      i < INSIGHT_PREDICT_PRO_WAVEFORM_DATA_END + 1;
      i++)
  {
    _attempt = 0;
        ...
        for(uint32_t blkCnt = 1; blkCnt < totalBlock; blkCnt++)
        {
               ...
               NULL) == true)
          {
            ...
            app_froto_data_upload_parser(readBuf,
                                         ...
                                         NULL);
            bulkDataBuf = bulkDataPro;
            if(bulkDataPro != NULL)
            {
              DBG_LOG_DEBUG("bulkDataPro->data @ %p, bulkDataPro->dataLen %d", 
                            bulkDataPro->data, bulkDataPro->dataLen);
            }
            pthread_mutex_unlock(&data_item_info_mtx);
            ...
            if(bulkDataBuf == NULL)
            {
              ...
              DBG_LOG_ERR("bulkDataPro @ %p, bulkDataBuf @ %p", bulkDataPro, bulkDataBuf);
            }
            else
            {
...
/**
 * @brief This is the configuration update instance of InsightPredictPro. In
...
_CONFIG_HASH_RETRIEVE:
     ...
     (sensorIntf->lastEditTimeDB > sensorIntf->lastEditTime))
  {
    ...
    for(uint32_t i = 2, // Skip the first two time-related configurations
        delta = INSIGHT_PREDICT_PRO_CONFIG_DISSEM_MAX_NUMBER;
        i < sizeof(specificItemGroup) / sizeof(specificItemGroup[0]);
       )
    {
      if((i + INSIGHT_PREDICT_PRO_CONFIG_DISSEM_MAX_NUMBER) >
      ...
      protobuf_codec_config_pair_ele_t _configPairEle[
        INSIGHT_PREDICT_PRO_CONFIG_DISSEM_MAX_NUMBER];
      protobuf_codec_config_pair_t _configPair;
...
_CONFIG_DISSEM:
  ...
  _attempt = 0;
  for(uint32_t i = 2, // Skip the first two time-related configurations
      delta = INSIGHT_PREDICT_PRO_CONFIG_RETRIEVE_MAX_NUMBER;
      i < sizeof(specificItemGroup) / sizeof(specificItemGroup[0]);
     )
  {
    if((i + INSIGHT_PREDICT_PRO_CONFIG_RETRIEVE_MAX_NUMBER) >
       sizeof(specificItemGroup) / sizeof(specificItemGroup[0]))
    {
      delta = sizeof(specificItemGroup) / sizeof(specificItemGroup[0]) -
              i;
...
/**
 * @brief This is the FUOTA instance of InsightPredictPro. In this routine, 1)
  ...
  uint8_t *err_code)
{
  ...
  if(pt->cnt == 1)
  {
    ...
    {
      bulkDataPro = l_malloc(sizeof(protobuf_codec_data_pair_ele_data_t));

      bulkDataPro->crc32 = pt->buffer[0].data.crc32;
      bulkDataPro->crc32_valid = pt->buffer[0].data.crc32_valid;
      bulkDataPro->dataLen = pt->buffer[0].data.dataLen;
      bulkDataPro->data = l_malloc(bulkDataPro->dataLen);
      memcpy(bulkDataPro->data, pt->buffer[0].data.data, bulkDataPro->dataLen);
      bulkDataPro->start_point = pt->buffer[0].data.start_point;
    }
  }
}
static bool
