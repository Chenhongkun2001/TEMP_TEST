...
static void start_dbus_daemon(void)
{
	...
	DBG_LOG_INFO("Lanuching dbus-daemon with conf-file:%s", prg_argv[3]);

	pid = fork();
	if (pid < 0) {
		l_error("failed to fork new process");
		DBG_LOG_INFO("failed to fork new process");
	...
	DBG_LOG_INFO("dbus-daemon process %d created", pid);
...
static void sigchld_handler(void *user_data)
{
	while (1) {
		...
		DBG_LOG_INFO("process %d terminated with status=%d", pid, status);
...
static void signal_message(struct l_dbus_message *message, void *user_data)
{
	...
	DBG_LOG_INFO("path=%s, destination=%s", path, destination);
		
	interface = l_dbus_message_get_interface(message);
	member = l_dbus_message_get_member(message);

	l_info("interface=%s member=%s", interface, member);
	DBG_LOG_INFO("interface=%s, member=%s", interface, member);

	sender = l_dbus_message_get_sender(message);

	l_info("sender=%s", sender);
	DBG_LOG_INFO("sender=%s", sender);
	...
	if (!strcmp(member, "NameOwnerChanged")) {
		...
		DBG_LOG_INFO("name=%s, old=%s, new=%s", name, old_owner, new_owner);
	}
}
							...
							void *user_data)
{
	...
	if (l_dbus_message_get_error(message, &error, &text)) {
		...
		DBG_LOG_INFO("error=%s", error);
		DBG_LOG_INFO("message=%s", text);
	...
	DBG_LOG_INFO("request_name result=%d", result);
