...
find_gpio_led_handle(const enum pin_number kp_pin_no)
{
  ...
  for(idx = 0; idx < (sizeof(g_gpio_led_handles) / sizeof(g_gpio_led_handles[0])); idx++)
  {
...
#else /* USING_AT_RPOCESS_SKF_GW == 1 */
    LOG_ERR(OUTPOINT, "Failed to open %s, for %s", kp_fpath, strerror(errno));
#endif /* USING_AT_RPOCESS_SKF_GW == 1 */
    tpret = ST_ERR;
  }
...
app_io_init_gpio_for_led(void)
{
  ...
  // Export the GPIO
  memset(command, 0, MAX_FPATH);
  kp_pin_no = PIN_NO_VOUT0_DATA14;
  snprintf(command,
           MAX_FPATH,
           STR_GPIO_EXPORT_PATTERN,
           kp_pin_no);
#if (USING_AT_RPOCESS_SKF_GW == 1)
  DBG_LOG_INFO("To exec %s", command);
#else /* USING_AT_RPOCESS_SKF_GW == 1 */
  LOG_INFO(OUTPOINT, "To exec %s", command);
#endif /* USING_AT_RPOCESS_SKF_GW == 1 */
  if((ST_ERR == is_gpio_exported(kp_pin_no)) && (system(command) != 0))
  {
#if (USING_AT_RPOCESS_SKF_GW == 1)
    DBG_LOG_WARN("Failed to execute the command, for %s", strerror(errno));
#else /* USING_AT_RPOCESS_SKF_GW == 1 */
    LOG_WARN(OUTPOINT, "Failed to execute the command, for %s",
             strerror(errno));
#endif /* USING_AT_RPOCESS_SKF_GW == 1 */
  }
  // Set the GPIO as output (to drive a LED)
  usleep(1000 * 100);
  memset(command, 0, MAX_FPATH);
  snprintf(command, MAX_FPATH, STR_GPIO_DIR_PATTERN, kp_pin_no);
#if (USING_AT_RPOCESS_SKF_GW == 1)
  DBG_LOG_INFO("To exec %s", command);
#else /* USING_AT_RPOCESS_SKF_GW == 1 */
  LOG_INFO(OUTPOINT, "To exec %s", command);
  ...
  {
#if (USING_AT_RPOCESS_SKF_GW == 1)
    DBG_LOG_ERR("Failed to execute the command, for %s", strerror(errno));
#else /* USING_AT_RPOCESS_SKF_GW == 1 */
    LOG_ERR(OUTPOINT, "Failed to execute the command, for %s",
            strerror(errno));
#endif /* USING_AT_RPOCESS_SKF_GW == 1 */
    tpret = ST_ERR;
    goto EXIT;
  ...
  enum led_state new_state)
{
  ...
  uint8_t kp_stval = 0;
  uint8_t kp_fpath[MAX_FPATH] = { 0 };
  struct gpio_led_handle *pt_handle = find_gpio_led_handle(kp_pin_no);
  int kp_fd = -1;
  bool close_fd_after_write = false;
  char kp_outbuf[2] = { '0', '\0' };
  ...
  kp_outbuf[0] = (char)('0' + kp_stval);

  if((NULL != pt_handle) && pt_handle->has_last_state &&
  ...
  if(NULL != pt_handle)
  {
    tpret = ensure_gpio_led_fd(pt_handle);
    if(ST_OK != tpret)
    {
      return tpret;
    }
    kp_fd = pt_handle->value_fd;
  }
  else
  {
    snprintf(kp_fpath, MAX_FPATH, STR_GPIO_VALUE_FPATH, kp_pin_no);
    kp_fd = open((const char *)kp_fpath, O_WRONLY);
    if(kp_fd < 0)
    {
#if (USING_AT_RPOCESS_SKF_GW == 1)
      DBG_LOG_ERR("Failed to open %s, for %s", kp_fpath, strerror(errno));
    ...
    close_fd_after_write = true;
  }

#if (USING_AT_RPOCESS_SKF_GW == 1)
  DBG_LOG_INFO("To write %s into gpio%d", kp_outbuf, kp_pin_no);
#else /* USING_AT_RPOCESS_SKF_GW == 1 */
  LOG_INFO(OUTPOINT, "To write %s into gpio%d", kp_outbuf, kp_pin_no);
#endif /* USING_AT_RPOCESS_SKF_GW == 1 */

  if(lseek(kp_fd, 0, SEEK_SET) < 0)
  {
#if (USING_AT_RPOCESS_SKF_GW == 1)
    DBG_LOG_ERR("Failed to seek gpio%d, for %s", kp_pin_no, strerror(errno));
#else /* USING_AT_RPOCESS_SKF_GW == 1 */
    LOG_ERR(OUTPOINT, "Failed to seek gpio%d, for %s", kp_pin_no, strerror(
              errno));
#endif /* USING_AT_RPOCESS_SKF_GW == 1 */
    tpret = ST_ERR;
  }

  if((ST_OK == tpret) && (write(kp_fd, kp_outbuf, 1) != 1))
  {
#if (USING_AT_RPOCESS_SKF_GW == 1)
    DBG_LOG_ERR("Failed to write gpio%d, for %s", kp_pin_no, strerror(errno));
#else /* USING_AT_RPOCESS_SKF_GW == 1 */
    LOG_ERR(OUTPOINT, "Failed to write gpio%d, for %s", kp_pin_no, strerror(
              errno));
#endif /* USING_AT_RPOCESS_SKF_GW == 1 */
    tpret = ST_ERR;
  }
