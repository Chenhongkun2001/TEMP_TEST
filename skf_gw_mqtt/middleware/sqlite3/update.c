...
static void constrcut_sql(uint32_t dataNO, gw_pro_data_t *dataptr, uint32_t tb,
                          char *sql);
static void gain_operadata(configure_content_t *tbitem, void *data, char *sql);
...
int sqlite_update(const char *dbname, uint32_t dataNO, uint32_t keyID,
  ...
  char sql[4096] = {0};
  char tempstr[100] = {0};
  snprintf(sql, sizeof(sql) - 1, "UPDATE %s SET ", tb);
  constrcut_sql(dataNO, dataptr, tbindex, sql);
  sql[strlen(sql) - 1] = ' ';
  if (strcmp(tb, SENSOR_DATA_TABLE))
    snprintf(tempstr, sizeof(tempstr) - 1, "where nameId = %d;", keyID);
  else
    snprintf(tempstr, sizeof(tempstr) - 1, "where SequenceNumber = %d;", keyID);
  strcat(sql, tempstr);
  ...
  if ((ret = sqlite3_exec(db, sql, NULL, NULL, &err_msg)) != SQLITE_OK) {
    ...
    LOG_INFO(OUTPOINT, "update successfully\n");
  }
